...
class KnowledgeService {
  ...
  async searchResources(query: string, limit: number = 5): Promise<any[]> {
  ...
  async getResourcesByCategory(categoryName: string, limit: number = 5): Promise<any[]> {
    const { data, error } = await this.supabase
      .from('ivor_resources')
      .select(`
        id,
  ...
  async getResourcesByTags(tags: string[], limit: number = 5): Promise<any[]> {
    const { data, error } = await this.supabase
      .from('ivor_resources')
      .select(`
        id,
  ...
  async fullTextSearch(query: string, limit: number = 5): Promise<any[]> {
    const { data, error } = await this.supabase
  ...
  formatResourcesForResponse(resources: any[], context: string = ''): string {
    ...
    resources.forEach((resource, index) => {
      ...
      const tags = resource.ivor_resource_tags?.map((rt: any) => rt.ivor_tags?.name).filter(Boolean) || []
  ...
  async getAllResources(limit: number = 100): Promise<any[]> {
    const { data, error } = await this.supabase
      .from('ivor_resources')
      .select(`
        id,
